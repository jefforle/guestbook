...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="entjr0/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/jefforle/guestbook.git'
            }
        }
        stage('Build') {
            steps {
        ...
        stage('Docker Image Push') {
            steps {
                script {
                    docker.withRegistry('', 'yul2720_DockerHub_Credential') {
                        oDockImage.push()
                    }
                }
            }
        ...
        stage('Staging Deploy') {
            steps {
                sshagent(credentials: ['yul2720-Staging-PrivateKey']) {
                    sh "ssh -o StrictHostKeyChecking=no root@172.31.0.110 docker container rm -f yul2720-guestbookapp"
                    sh "ssh -o StrictHostKeyChecking=no root@172.31.0.110 docker container run \
                                        -d \
                                        -p 32720:80 \
                                        --name=yul2720-guestbookapp \
        ...
        stage ('JMeter LoadTest') {
            steps { 
                sh '~/yul2720/lab/sw/jmeter/bin/jmeter.sh -j jmeter.save.saveservice.output_format=xml -n -t src/main/jmx/guestbook_loadtest.jmx -l loadtest_result.jtl' 
    ...
    post { 
        always { 
            emailext (attachLog: true, body: '본문', compressLog: true
                    , recipientProviders: [buildUser()], subject: '제목', to: 'jefforle20@gmail.com')
